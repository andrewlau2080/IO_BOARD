...
/** @addtogroup 456_AES_CTR_USING_DMA
  * @{
  */
...
flag_status crypt_done = RESET;

static void aes_dma_config(uint8_t* out_buffer, uint8_t* in_buffer, uint32_t sz);
void aes_encrypt_process(uint8_t* encrypt_buf, uint8_t* original_buf, uint32_t buf_len);
void aes_decrypt_process(uint8_t* decrypt_buf, uint8_t* encrypt_buf, uint32_t buf_len);  
...
/**
  * @brief  aes encrypt process.
  * @param  encrypt_buf: pointer to the encrypt buf
  *         original_buf: pointer to the original buf
  *         buf_len: the data length, unit in words.
  * @retval none
  */
void aes_encrypt_process(uint8_t* encrypt_buf, uint8_t* original_buf, uint32_t buf_len)
{
  ...
  aes_interrupt_enable(AES_PDIE_INT | AES_RWEIE_INT, TRUE);
  crypt_done = RESET;
  ...
  while(crypt_done != SET);
}

/**
  * @brief  aes decrypt process.
  * @param  decrypt_buf: pointer to the decrypt buf
  *         encrypt_buf: pointer to the encrypt buf
  *         buf_len: the data length, unit in words.
  * @retval none
  */
void aes_decrypt_process(uint8_t* decrypt_buf, uint8_t* encrypt_buf, uint32_t buf_len)
...
int main(void)
{
  ...
  /* nvic and interrupt config */
  nvic_priority_group_config(NVIC_PRIORITY_GROUP_4);
  nvic_irq_enable(AES_IRQn, 1, 0);

  /* data encrypt */
  aes_encrypt_process((uint8_t *)encrypt_buf, (uint8_t *)original_buf, DATA_LENGTH);

  /* data decrypt */
  aes_decrypt_process((uint8_t *)decrypt_buf, (uint8_t *)encrypt_buf, DATA_LENGTH);
  
  if(memcmp(decrypt_buf, original_buf, DATA_LENGTH) != 0)
  {
    err = 1;
  }
