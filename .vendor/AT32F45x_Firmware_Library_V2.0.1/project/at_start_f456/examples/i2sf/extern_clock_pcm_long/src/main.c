...
/** @addtogroup 456_I2SF_extern_clock_pcm_long I2SF_extern_clock_pcm_long
  * @{
  */
...
static void spi_i2s_dma_config(void)
{
  ...
  dmamux_init(DMA1MUX_CHANNEL2, DMAMUX_DMAREQ_ID_I2S2_EXT_TX);
}

/**
  * @brief  spi_i2s configuration.
  * @param  none
  * @retval none
  */
static void spi_i2s_config(void)
{
  ...
  crm_periph_clock_enable(CRM_I2S2EXT_PERIPH_CLOCK, TRUE);
  ...
  spi_i2s_dma_transmitter_enable(I2S2EXT, TRUE);
...
/**
  * @brief  spi_i2s gpio configuration.
  * @param  none
  * @retval none
  */
...
static void i2sf_gpio_config(void)
{
  ...
  gpio_init(GPIOB, &gpio_initstructure);
  gpio_pin_mux_config(GPIOB, GPIO_PINS_SOURCE6, GPIO_MUX_13);

  /* i2sf ck pin */
  gpio_initstructure.gpio_pull = GPIO_PULL_DOWN;
  gpio_initstructure.gpio_pins = GPIO_PINS_7;
  gpio_init(GPIOB, &gpio_initstructure);
  gpio_pin_mux_config(GPIOB, GPIO_PINS_SOURCE7, GPIO_MUX_13);
  ...
  gpio_init(GPIOB, &gpio_initstructure);
  gpio_pin_mux_config(GPIOB, GPIO_PINS_SOURCE9, GPIO_MUX_13);

  /* i2sf sdext pin */
  gpio_initstructure.gpio_pull = GPIO_PULL_DOWN;
  gpio_initstructure.gpio_pins = GPIO_PINS_8;
  gpio_init(GPIOB, &gpio_initstructure);
  gpio_pin_mux_config(GPIOB, GPIO_PINS_SOURCE8, GPIO_MUX_13);
