...
/** @addtogroup 457_CAN_time_stamping CAN_time_stamping
  * @{
  */
...
#define FD_BTS1_SIZE                     18
#define FD_BTS2_SIZE                     6
#define FD_RSAW_SIZE                     6
#define FD_SSP_OFFSET                    19
...
/* the calculation process of data bit rate is as follows: 
   fd_baudrate = can_clk/(FD_BTS1_SIZE + FD_BTS2_SIZE) = 192M/(18 + 6) = 8M;
   fd_samplepoint = FD_BTS1_SIZE/(FD_BTS1_SIZE + FD_BTS2_SIZE) = 18/24 = 75%;
*/
...
static void tmr_configuration(void)
{
  ...
  tmr_base_init(TMR5, 0xFFFFFFFF, (crm_clocks_freq_struct.apb1_freq / 1000000) - 1);
...
static void scfg_configuration(void)
{
  ...
  scfg_can_timestamp_source_set(SCFG_CAN1, SCFG_CAN_TIMESTAMP_TMR5);
}
