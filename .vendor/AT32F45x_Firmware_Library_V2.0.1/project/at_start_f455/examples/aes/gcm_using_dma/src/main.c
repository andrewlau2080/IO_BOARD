...
/** @addtogroup 455_AES_GCM_USING_DMA
  * @{
  */
...
static void aes_dma_config(uint8_t* out_buffer, uint8_t* in_buffer, uint32_t sz, aes_processing_stage_type prc_type);
...
/**
  ...
  *         prc_type: processing stage type
...
static void aes_dma_config(uint8_t* out_buffer, uint8_t* in_buffer, uint32_t sz, aes_processing_stage_type prc_type)
{
  ...
  if(prc_type == AES_PRC_DATA)  /* operate data in multiples of 16 bytes only */
  {
    sz /= 16;
    sz *= 4;
  }
...
void aes_decrypt_process(uint8_t* decrypt_buf, uint8_t* encrypt_buf, uint32_t buf_len, uint8_t* tag_buf)
{
  ...
  if(buf_len / 16 > 0)
  {
    /* dma is used when data length at least 16 bytes */
    aes_dummy_data_num_set(0);
    aes_dma_config(decrypt_buf, encrypt_buf, buf_len, AES_PRC_DATA);
    aes_dma_in_enable(TRUE);
    aes_dma_out_enable(TRUE);
    aes_enable(TRUE);
    while(aes_flag_get(AES_PDFS_FLAG) == RESET);
