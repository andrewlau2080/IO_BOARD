...
#define ADDR_SHIFT(A)                    (BANK1_NOR4_ADDR + (2 * (A)))
...
void nor_init(void)
{
  ...
  gpio_pin_mux_config(GPIOG, GPIO_PINS_SOURCE12, GPIO_MUX_12);
...
nor_status nor_eraseblock(uint32_t blockaddr)
{
    ...
    NOR_WRITE(ADDR_SHIFT(0x0555), 0x00AA);
    NOR_WRITE(ADDR_SHIFT(0x02AA), 0x0055);
    NOR_WRITE((BANK1_NOR4_ADDR + blockaddr), 0x30);
...
nor_status nor_writehalfword(uint32_t writeaddr, uint16_t data)
{
    ...
    NOR_WRITE((BANK1_NOR4_ADDR + writeaddr), data);
...
uint16_t nor_readhalfword(uint32_t readaddr)
{
    ...
    NOR_WRITE((BANK1_NOR4_ADDR + readaddr), 0x00F0 );

    return (*(__IO uint16_t *)((BANK1_NOR4_ADDR + readaddr)));
}
...
void nor_readbuffer(uint16_t* pbuffer, uint32_t readaddr, uint32_t numhalfwordtoread)
{
    ...
    for(; numhalfwordtoread != 0x00; numhalfwordtoread--) /*!< while there is data to read */
    {
        ...
        *pbuffer++ = *(__IO uint16_t *)((BANK1_NOR4_ADDR + readaddr));
        readaddr = readaddr + 2;
    }
}
...
nor_status nor_getstatus(uint32_t timeout)
{
    ...
    while((timeout != 0x00) && (status != NOR_SUCCESS))
    {
        ...
        /*!< read dq6 and dq5 */
        val1 = *(__IO uint16_t *)(BANK1_NOR4_ADDR);
        val2 = *(__IO uint16_t *)(BANK1_NOR4_ADDR);
