...
/** @addtogroup AT32F45x_middlewares_usbd_drivers
  * @{
  */
...
{
  /* after running usbd_remote_wakeup(),the host did not wake up immediately,
   causing the device to enter the suspended state again.
   the device cannot detect the host resume signal, so use the sof clear the suspend state.*/
