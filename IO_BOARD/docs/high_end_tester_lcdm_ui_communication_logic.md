...
单个 IN/OUT 格由黑色边框、两块背景色和两段文字组成。黑格、白格、缺号的
根因不是字号，而是 MCU 连续发送大量 `fill`/`xstr` 时，LCDM 尚未完成前一条
字形渲染就收到后续命令，后续文字命令会滞后或丢失，只留下先完成的边框/底色。

固件在网格绘制前启用 `bkcmd=3`：每条绘图指令收到 LCDM 的 `0x01` 成功回执后
才发送下一条，并使用 250 ms 的回执窗口。大网格**不能**使用 `ref_stop` /
`ref_star` 累积到屏幕的延迟刷新队列。学习过程只显示当前扫描进度；连线颜色在
完整矩阵扫描结束后统一绘制一次，避免反复重画已完成格。
...
普通测试页仍只把底部带映射为 K1-K4；其它坐标必须保持无业务动作。高档 LCDM
测试机在 `READY`/安全复位待机时，K3 按住约 3 秒即进入本机设置页（达到阈值立即绘制，
不等待松手；随后释放事件不执行动作）。该页临时解锁
其它触控区域，用于机身编号、线体/工位、WiFi SSID/密码和本线打印主机 TCP 地址/端口
的人工输入；K3 短按取消并回到普通测试页，K4 保存。K1 长按学习、K2 自动测试含义
不变。

设置页沿用既有 LCDM 字体资源（标题 Font ID 2、小字 Font ID 4）；字段过长时只缩短
标签文字，不替换字体。密码在生产设置页明文显示，输入字符只刷新编辑栏的变化后缀。

完整字段、Flash 双副本、TJC 文本命令和 ESP-AT 连接/回滚流程见
