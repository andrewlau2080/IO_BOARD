...
0. **测试机 ESP 会话中途复位后乱码（2026-08-06 22:15，烧录引发，待断电恢复）**：
   - 现象：烧录测试机（under-reset 复位 AT32，ESP 在线会话中途）后，测试机 RX 边沿 63K+
     但解码 0 帧 0 错误 —— ESP 输出无法解码的乱码流量（非 115200 数据）。
   - 打印侧同固件解码正常（25 帧）→ 解码代码无恙；测试机固件 6/6 校验点（含尾部）一致 →
     固件无恙。判定为 ESP 模块在 AT32 复位后的瞬时异常状态。
   - 处理：测试机彻底断电 10s 再上电（ESP 冷启动复位）后应恢复。**待用户操作后复核。**
   - 教训：**烧录（尤其 under-reset）会在 ESP 在线时复位 AT32 → ESP 可能进入乱码态；
     烧录后若异常，先断电重启 ESP 再排查，不要先怀疑代码。**
1. **打印侧 ESP 掉电后沉默（2026-08-06 晚）**：
   ...
   - **"进设置页才能连上"的机制核查（2026-08-06 21:50，用户关键观察）**：
     - 用户观察：上电不连 → 进设置页 → 退出后 WIFI 正常。该现象此前多次出现。
     - 核查结论：**设置页不是修复机制**。设置页对 ESP 只有两处 `print_host_wifi_restart()`
       （COMM 页 K1 "WIFI TEST"、保存 COMM 改动时），它与自动重试路径调用的是**同一个**
       `print_host_wifi_start()`（at_begin→踢活+++→CRLF→AT→…），无任何额外 ESP 操作。
     - 实锤：本次"恢复"后读 RAM —— host=ONLINE、**reconnect=0（计数清零 = AT32 重新上电过）**、
       25 帧、2532 边沿、0 解码错误 → 断电重启后**首次会话即成功**。用户进设置页时 ESP 已恢复，
       K1/退出只是撞上了恢复后的时刻。
     - 所以完整逻辑：打印侧 ESP **上电偶发不启动**（本次沉默 12 分钟：rx 边沿/帧/错误全 0 =
       ESP 从未发出任何信号，期间自动重试的 kick+AT 均无回应）→ 断电重启后 ESP 正常启动 →
       首次会话成功。测试机 ESP 上电稳定，故无此现象。**两侧程序完全一致，差异在 ESP 模块
       的上电可靠性**（偶发不启动，倾向 flash/模块问题）。
   - **两侧连接程序一致性已做寄存器级验证（2026-08-06 21:30，回答"为什么测试机没问题"）**：
     | 检查项 | 测试机(407) | 打印侧(807) |
     |---|---|---|
     | EXINT line9 使能 inten (0x40013C00) | 0x200 | 0x200 |
     | EXINT 边沿 polcfg1/polcfg2 | 0x200/0x200 | 0x200/0x200 |
     | NVIC EXINT9_5 (ISER0 bit23) | 使能 | 使能 |
     | PB9 RX 电平 (GPIOB_IDT bit9) | 高(空闲) | 高(空闲) |
     | PC3 TX (GPIOC CFGCTL/IDT/ODT) | 输出/高/高 | 输出/高/高 |
     结论：两侧运行完全相同的连接程序（同一 tester_wifi_print.c，寄存器实测一致），
     ESP 的差异在模块本身 —— 打印侧 ESP 从未发出任何信号（连启动日志都没有）。
