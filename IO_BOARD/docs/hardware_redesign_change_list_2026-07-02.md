...
目的：给重新设计原理图和 PCB layout 使用。当前核对依据以 `SCH_FIXTURE_2026-07-05.pdf` 为准；旧 `SCH_FIXTURE_2026-06-30.pdf` 只作历史对照。产品规划：扫描 IO 模块、打印接收/LCDM 模块可编译在同一固件，但实际产品中独立使用。
...
|---:|---|---|---|---|---|
| 1 | `HALL_SW` / 打印感应触发 | `PB8` | 按 2026-07-05 图纸接霍尔/感应器输出；接口建议 3.3V、GND、HALL_SW | `PC7` 是按键 `KEY_RIGHT`，此前只作临时触发测试，正式版本不得再占用 | 必须 |
| 2 | 触发电平 | `PB8` 输入上拉，低电平有效 | 传感器输出用开漏/集电极开路下拉，或 3.3V TTL；必要时加 100R-1k 串阻和 ESD/RC 位置 | AT32 GPIO 不允许 5V 直接输入；低有效与当前固件一致 | 必须 |
| 3 | 与旧裸屏冲突 | `PB8` 旧图曾作 `LCM_BL_LED` | 2026-07-05 图纸已改为 `HALL_SW`；不要再把 PB8 当 LCD 背光输出 | 避免 PB8 同时作为输出背光和输入触发 | 必须 |

## 红外打印通讯

| 序号 | 信号/模块 | 当前临时脚 | 建议最终硬件 | 原因/说明 | 优先级 |
|---:|---|---|---|---|---|
| 1 | IR_TX | PB6 | 按 2026-07-05 图纸接 IR 发射驱动；必须用三极管/MOSFET 驱动 IR LED，不要 MCU 直接带 LED | 固件已按 `PB6=IR_TX` 更新；正式建议用硬件 PWM 载波 + 包络开关 | 必须 |
...
| 2 | LCDM 串口 | `PB3=LCDM_TX`、`PB5=LCDM_RX` | 按 2026-07-05 图纸接 `J_LCDM_TJC`：VCC、GND、LCDM_RX、LCDM_TX；MCU TX 接屏 RX，MCU RX 接屏 TX | TJC 不要求固定硬件 USART；使用软件 UART，避免占用 `PA9/PA10`，且不占用 `PA2/ADC2_IN2` 或封装未确认的 `PH2` | 必须 |
| 3 | LCDM 电源/电平 | 按屏模块要求供电，串口 3.3V 兼容 | 若屏为 5V 供电但串口不是 3.3V 兼容，必须加电平转换 | AT32 GPIO 不能直接承受 5V 输入 | 必须 |
| 4 | 旧裸屏接口 | PB3/SCK、PB5/MOSI、PB4/RESET、PB6/CMD、PB7/CS、PB8/BL | 2026-07-05 图纸已改为：PB3/PB5=TJC LCDM，PB6/PB7=IR，PB8=HALL_SW；旧裸屏接口不再作为当前方案 | 若最终改回裸屏，固件要另写 SPI LCD + 触摸 + 软键盘，并重分配 IR/感应触发脚 | 建议 |
...
| 8 | WiFi/无线模块 | 模块未定，当前不分配固定 MCU IO | 只预留板边安装空间、电源余量和可选 0R/测试点；等模块确认后再定 UART/SPI/SDIO 与 EN/RST 脚 | 目前还没有找到合适 WiFi 模块，不能把 ESP8266/ESP32 或某组 UART 脚写成最终规格 | 待定 |
...
| IR 打印测试 | PB6=IR_TX、PB7=IR_RX | 按 2026-07-05 图纸更新；正式建议硬件 PWM 载波 |
| WiFi/无线模块 | 未定义 | 当前没有选定合适模块，不分配 MCU IO；后续按实物模块另行更新 |
| 扫描输出 | PA4/PA5 | 数字输出激励 |
