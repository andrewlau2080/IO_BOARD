...
本次按 `docs/界面说明260722.xlsx` 继续整理 LCDM 高档测试机界面：

- 自动学习流程改为 94 点扫描，1-48 使用 A 半区输入，49-94 使用 B 半区输入。
- 学习过程显示 `LEARNING` 和 `NNN - NNN`，步进时间为 0.1 秒。
- 学习完成页显示 `LEARNING OUTCOME`，结果区左右分栏，标题闪烁时不重画结果分隔线。
- K4 确认后停留在 `CONFIRMED` 页，下一次按 K1-K4 再执行对应功能。
- 单按 K1 的自检结果表按两页显示到 `IN/OUT094`，检测后逐格变色。
- K2 自动测试入口显示 `AUTO TESTING` 和当前线序，自动测试步进改为 50 ms。
- PASS 后不再无条件跳到打印界面；只有 PB8 磁感触发后才进入打印流程。
- PASS 结果页使用左侧大字 `PASS`，右侧三行显示 `TOTAL`、`NNN PAIRS`、`NNN POINTS`。
- 顶部标题保留稳定显示 `STANDARD CABLE`，不使用带空格的大字体写法，避免 LCDM 字符显示异常。

本次已编译并下载 `build-lcdm-tester` 到板子。现场后续重点继续核对：

- PASS 字体和标题栏视觉大小是否还需要微调。
- NG 与打印后续页面是否完全按 `界面说明260722.xlsx` 继续整理。
